...
export function setMarkerCenter(markers, format) {
  format = format ? format : ['lng', 'lat']
  let stationData = markers;
  ...
  if (stationData.length) {
    ...
    for (let i = 0; i < stationData.length; i++) {
      let lng = stationData[i].lng * 1;
      let lat = stationData[i].lat * 1;
      lngSum += lng;
      latSum += lat;
    }
...
export function json2Geojson(arr, format) {
  ...
  let lnglat = Object.assign(defaultLnglat, format)
  var geojson = {
